...
응용 프로그램 마법사에서 이 Gaia 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

Gaia 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
Gaia.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 GaiaApp 응용 프로그램 클래스가 선언되어 있습니다.

Gaia.cpp
    이는 응용 프로그램 클래스 GaiaApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\Gaia.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 Gaia.rc에 의해 포함됩니다.
...
GaiaDoc.h, GaiaDoc.cpp - 문서
    이 파일에는 GaiaDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 GaiaDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      gaia
        파일 형식 ID:        Gaia.Document
        주 프레임 캡션:  Gaia
        Doc 형식 이름:       Gaia
        필터 이름:         Gaia Files (*.gaia)
        파일의 새 약식 이름: Gaia
        파일 형식의 긴 이름: Gaia.Document

GaiaView.h, GaiaView.cpp - 문서 뷰
    이 파일에는 GaiaView 클래스가 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 Gaia.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.
