...
void OrGate::Calculate(){
	...
	else if (db[this->ins[1].CenterPoint().x / 10][this->ins[1].CenterPoint().y / 10] == 1){
		db[this->outs[0].CenterPoint().x / 10][this->outs[0].CenterPoint().y / 10] = 1;
	}
	else if (db[this->ins[0].CenterPoint().x / 10][this->ins[0].CenterPoint().y / 10] == 0 && db[this->ins[1].CenterPoint().x / 10][this->ins[1].CenterPoint().y / 10] == 0){
...
IMPLEMENT_SERIAL(OrGate, GaiaLLogic, 7)
void OrGate::Serialize(CArchive& ar){
